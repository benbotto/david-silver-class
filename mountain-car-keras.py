    ...
    while not done:
      ...
      if seqSolveCt < FIN_TRAIN_SOLVE_CT:
        ...
        for i in range(len(batch)):
          ...
          #target[batch[i]['action']] = batch[i]['reward'] + GAMMA * np.max(newQ)
          #target[batch[i]['action']] = batch[i]['newObs'][0] + GAMMA * np.max(newQ)
          # Best reward so far.
          target[batch[i]['action']] = batch[i]['maxPos'] + GAMMA * np.max(newQ)
          #target[batch[i]['action']] = batch[i]['maxPos'] + batch[i]['reward'] + GAMMA * np.max(newQ)
          Y.append(target)
